...
/**An unbeatableAI implemented using the minimax algorithm*/
public class UnbeatableAI {
    ...
    public void nextMove(final PlayerVsAI_3x3_Activity board) {
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                MinMaxScoreCalculator bestMove = minimax(textBoard, level, 0, 0); // (textBoard, level, recurse, depth)
    ...
    private MinMaxScoreCalculator getResult(ArrayList<MinMaxScoreCalculator> listScore, String level) {
        MinMaxScoreCalculator result = listScore.get(0);
    ...
    private MinMaxScoreCalculator minimax(String[] textBoard, String level, int recurse, int depth) {
        ...
        if(children == null || gameOver(textBoard)) {
            return new MinMaxScoreCalculator(textBoard, getScore(textBoard), depth); // if no children or game over
        } else {
            ArrayList<MinMaxScoreCalculator> listScore = new ArrayList<>(); // Get a list of scores for all available children
            for(int i = 0; i < children.size(); i++) {
                listScore.add(minimax(children.get(i), invertLevel(level), 1, depth + 1)); // recursive call
                                                                                           // with recurse = 1 and depth + 1
            }

            MinMaxScoreCalculator res = getResult(listScore, level); // Get the child
